...
export const facebook = async (link: string) => {
  try {
    let res = await getFbVideoInfo(link);
    console.log("res-->>", res);
    return { status: 200, data: res };
  } catch (error:any) {
    console.error(error);
    return {
        data: `error in instagram function: ${error.message}`,
        success: false,
        statusCode: 500,
      };
  }
};
const getFbVideoInfo = async (videoUrl: string) => {
